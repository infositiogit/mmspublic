...
public class GingerbreadJSFixExample {
	 ...
	 */
	public void fixWebViewJSInterface(WebView webview, Object jsInterface, String jsInterfaceName, String jsSignature, MMSActivity act) {
		// Gingerbread specific code
		if (Build.VERSION.RELEASE.startsWith("2.3")) {
			javascriptInterfaceBroken = true;
		} else { // Everything else is fine
			Log.i("testmms", "Interface normal");
			webview.addJavascriptInterface(jsInterface, jsInterfaceName);
		}

		webview.setWebViewClient(new GingerbreadWebViewClient(jsInterface, jsInterfaceName, jsSignature, act));
	...
	private class GingerbreadWebViewClient extends WebViewClient {
		...
		private MMSActivity mmsAct;

		public GingerbreadWebViewClient(Object jsInterface, String jsInterfaceName, String jsSignature, MMSActivity act) {
		...
		public boolean shouldOverrideUrlLoading(WebView view, String url) {
		    if (url.startsWith("mailto:")) {  
		        //Handle mail Urls 
		        mmsAct.startActivity(new Intent(Intent.ACTION_SENDTO, Uri.parse(url)));
		    } else if (url.startsWith("tel:")) {
		        //Handle telephony Urls 
		        mmsAct.startActivity(new Intent(Intent.ACTION_DIAL, Uri.parse(url)));
		    } else { 
		        view.loadUrl(url);
		    } 
		    return true; 
